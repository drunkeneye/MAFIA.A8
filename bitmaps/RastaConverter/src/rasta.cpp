...
void RastaConverter::FindBestSolution()
{
	...
	fmtmessage(0, 300, "Evaluations: %u", m_evaluations);
...
#else
	while(m_evaluations < cfg.max_evals && m_best_result != 0 && m_stalled == 0)
#endif
	{
		...
		// check for progress
		if ((m_evaluations - m_last_best_evaluation) > 1234567 ) {
			// this doesnt work
			Message ("No progress");
			m_stalled = 1;
			Wait(1);
		...
		if (m_evaluations%eval_period==0)
		{
			...
			fmtmessage(0, 300, "Evaluations: %10llu  LastBest: %10llu  Cached ins: %8u Rate: %6.1f", m_evaluations,m_last_best_evaluation, line_cache_insn_pool_level, rate);
			fmtmessage(0, 310, "Norm. Dist: %f", NormalizeScore(m_best_result));
		}
